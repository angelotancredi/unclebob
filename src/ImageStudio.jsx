...
export default function ImageStudio() {
  ...
  const processImage = useCallback(async () => {
    ...
    try {
      ...
      if (bgMode === "ai") {
        // AI mode: remove bg at ORIGINAL resolution first (AI works best on clean input)
        // Then upscale the result — this gives much better quality than upscale-then-AI
        setStep("AI 모델 로딩 중...");
        setProgress(30);

        const { removeBackground } = await import("@imgly/background-removal");

        setStep("AI 배경 분석 중 (고품질)...");
        setProgress(40);

        // Use original image (not upscaled) for AI — cleaner input = better result
        const origBlob = await new Promise(res => original.toBlob(res, "image/png"));
        const aiResult = await removeBackground(origBlob, {
          model: "medium",
          output: {
            format: "image/png",
            quality: 1.0,
          ...
          progress: (key, current, total) => {
            if (key === "compute:inference") {
              setStep("AI 피사체 분리 중...");
              setProgress(40 + Math.round((current / total) * 30));
            }
          }
        });
        ...
        // NOW upscale the clean bg-removed result
        setStep("고품질 확대 중...");
        setProgress(75);
        nobg = settings.scale > 1 ? ImageEngine.upscale(aiCanvas, settings.scale) : aiCanvas;

      } else {
        // Fast mode: upscale first, then color-based removal
        setStep("배경 제거 중...");
        await new Promise(r => setTimeout(r, 100));
